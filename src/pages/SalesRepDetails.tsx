...
export const SalesRepDetails: React.FC = () => {
  ...
  return (
    <div className="p-4 max-w-full mx-auto space-y-6">
      ...
      <Card className="space-y-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                {assignedClients.map(client => (
                  <tr key={client.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <td className="px-4 py-2 text-gray-900 dark:text-gray-100">{client.status === 'active' ? 'نشط' : client.status === 'inactive' ? 'غير نشط' : client.status === 'prospect' ? 'عميل محتمل' : 'عميل رائد'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
